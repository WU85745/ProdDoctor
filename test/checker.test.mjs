...
test('关键字缺失应失败', () => {
  const result = classifyResponse({ status: 200, body: '<h1>Wrong</h1>', expected: 'Expected' });
  assert.equal(result.ok, false);
...
test('可以检查本地真实 HTTP 服务', async (t) => {
  ...
  const result = await runChecks(`http://127.0.0.1:${port}`, { expected: 'ProdDoctor test marker', retries: 0, timeoutMs: 2000 });
  assert.equal(result.ok, true);
