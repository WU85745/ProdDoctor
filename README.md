...
![Version](https://img.shields.io/badge/version-v2.1.1-2563eb)
...
```yaml
- uses: lucaswenbo/ProdDoctor@v2.1.1
  with:
...
> v2.1.1 keeps lightweight HTTP validation as the default. Enable browser mode only when you need real Chromium execution, mobile evidence, screenshots, traces, or rendered-page assertions.
...
v2.1.1 stores browser evidence in an isolated artifact:
...
The current public stable release is **v2.1.1**.

- For normal evaluation and first-time integration, use `lucaswenbo/ProdDoctor@v2.1.1`.
...
- Concrete release tags such as `v2.1.1` are immutable after publication. Fixes should be released as a new patch, for example `v2.1.2`.
...
| `lucaswenbo/ProdDoctor@v2.1.1` | Recommended starting point | Concrete release tag; immutable by project policy |
...
Do not copy `<commit-sha>` literally. Open the GitHub Release or tag page for `v2.1.1`, follow it to the corresponding commit, and copy the full SHA. If you have the tag locally, you can also run:
