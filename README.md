...
> v1.5.0 keeps lightweight HTTP validation as the default. Enable browser mode only when you need real Chromium execution, mobile evidence, screenshots, traces, or rendered-page assertions.
...
v1.5.0 stores browser evidence in an isolated artifact:
...
The current public stable release is **v1.5.0**.

- For normal evaluation and first-time integration, use `lucaswenbo/ProdDoctor@v1.5.0`.
...
- Concrete release tags such as `v1.5.0` are immutable after publication. Fixes should be released as a new patch, for example `v1.5.1`.
...
| `lucaswenbo/ProdDoctor@v1.5.0` | Recommended starting point | Concrete release tag; immutable by project policy |
...
Do not copy `<commit-sha>` literally. Open the GitHub Release or tag page for `v1.5.0`, follow it to the corresponding commit, and copy the full SHA. If you have the tag locally, you can also run:
