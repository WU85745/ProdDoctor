...
> v2.0.1 默认仍保持轻量 HTTP 检查；需要时可以启用 Playwright + Chromium 浏览器模式。浏览器模式现在还可以生成移动端证据、失败 Trace、整页截图，以及独立的 HTML / JSON Production Report。
...
v2.0.1 会把浏览器证据集中放在一个 Artifact 中：
...
当前对外稳定版本为 **v2.0.1**。用户示例默认引用具体版本 `@v2.0.1`；如果 Releases / tag 页面尚未出现该版本，请先不要把这个引用用于实际流水线。

- 功能仍可能较快增加，升级前请先查看 [CHANGELOG.md](CHANGELOG.md)。
- 一般试用或首次接入，使用 `lucaswenbo/ProdDoctor@v2.0.1`。
- 生产门禁建议把具体 tag 换成该 tag 对应的**完整 commit SHA**，避免任何引用漂移。
- `@main` 跟踪最新开发代码，行为可能随时变化，不建议用于生产门禁。
- 具体版本 tag（例如 `v2.0.1`）发布后禁止移动；补丁修复应发布新的 patch 版本，例如 `v2.0.2`。
...
| `lucaswenbo/ProdDoctor@v2.0.1` | 推荐入门和一般项目接入 | 具体版本 tag，按项目约定发布后不移动 |
...
不要把 `<commit-sha>` 原样复制。发布 `v2.0.1` 后，可以从 GitHub **Releases / v2.0.1 tag 页面**进入该版本对应的 commit，再复制完整 SHA；本地已拉取 tag 时也可以运行：
