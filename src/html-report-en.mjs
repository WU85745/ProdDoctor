...
function statusClass(ok, warning = false) { if (warning) return 'warn'; return ok ? 'ok' : 'bad'; }
...
export function toEnglishHtmlReport(result, options = {}) {
  ...
  const rows = [
    row('DNS', result.dns.ok, result.dns.ok ? result.dns.addresses.map((item) => item.address).join(', ') : result.dns.error || 'failed'),
    row('Production page', result.page.ok, `HTTP ${result.page.status ?? 'no response'} · ${result.page.elapsedMs}ms`),
    row('Cloudflare/WAF', !result.page.blockedByChallenge, result.page.blockedByChallenge ? 'Likely challenge page detected' : 'No typical block detected'),
    row('TLS', !result.tls.checked || result.tls.ok, result.tls.checked
      ? `${result.tls.authorized ? 'certificate chain valid' : result.tls.authorizationError || 'invalid'} · ${result.tls.daysRemaining ?? '?'} days`
      : 'not checked'),
    row('Same-origin JS/CSS', !result.assets.checked || result.assets.ok, result.assets.checked
      ? `${result.assets.count} checked, ${result.assets.failedCount} failed`
      : result.assets.reason || 'disabled'),
    row('Browser', !browser.checked || browser.ok, browser.checked
      ? `${browser.profile} · HTTP ${browser.mainStatus ?? 'no response'} · ${browser.viewport?.width ?? '?'}×${browser.viewport?.height ?? '?'}`
      : 'disabled'),
    row('robots.txt', result.auxiliary.robots.ok, `HTTP ${result.auxiliary.robots.status ?? 'no response'}`, !result.auxiliary.robots.ok),
    row('sitemap.xml', result.auxiliary.sitemap.ok, `HTTP ${result.auxiliary.sitemap.status ?? 'no response'}`, !result.auxiliary.sitemap.ok)
  ].join('');
  ...
  const browserEvidence = browser.checked ? `
      <section>
        ...
        ${screenshotName ? `<h3>Screenshot</h3><p><a href="./${esc(screenshotName)}">${esc(screenshotName)}</a></p><img class="screenshot" src="./${esc(screenshotName)}" alt="ProdDoctor browser screenshot">` : ''}
        ${traceName ? `<h3>Playwright Trace</h3><p><a href="./${esc(traceName)}">${esc(traceName)}</a></p>` : ''}
        <h3>Uncaught JavaScript errors</h3>
        ${list(browser.pageErrors)}
        <h3>Console errors</h3>
        ${list(browser.consoleErrors)}
        <h3>Critical same-origin request failures</h3>
        ${list(browser.criticalRequestFailures.map((item) => `${item.resourceType}: ${item.url} · ${item.errorText}`))}
        <h3>Critical same-origin 4xx/5xx</h3>
...
<head>
  ...
  <style>
    ...
    h1 { margin-bottom: 8px; } h2 { margin-top: 36px; } h3 { margin-top: 24px; }
    ...
    .summary strong { font-size: 20px; } .ok-text { color: #5ee28c; } .bad-text { color: #ff7070; }
    ...
    .pill.ok { background: #163b26; color: #7df0a7; } .pill.bad { background: #481d1d; color: #ff9797; } .pill.warn { background: #463816; color: #ffd66e; }
    ...
    .card span { font-size: 20px; } a { color: #7eb8ff; } code { background: #1b2129; padding: 2px 5px; border-radius: 5px; }
    ul { padding-left: 22px; } li { margin: 6px 0; overflow-wrap: anywhere; }
...
<body><main>
  ...
  <div class="summary"><strong class="${result.ok ? 'ok-text' : 'bad-text'}">${result.ok ? 'PASS' : 'FAIL'}</strong><div class="muted">ProdDoctor v${esc(result.version)}</div></div>
  <section><h2>Check results</h2><table><thead><tr><th>Check</th><th>Status</th><th>Details</th></tr></thead><tbody>${rows}</tbody></table></section>
  ${browserEvidence}
  <section><h2>Blocking issues</h2>${list(result.failures)}</section>
  <section><h2>Warnings</h2>${list(result.warnings)}</section>
</main></body></html>`;
}
